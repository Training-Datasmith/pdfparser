 ...
 * Class ElementDate
 */
class Element_Date extends Element_String
{
    /**
     * @var array<int,string>
     */
    protected static $formats = [4 => 'Y', 6 => 'Ym', 8 => 'Ymd', 10 => 'YmdH', 12 => 'YmdHi', 14 => 'YmdHis', 15 => 'YmdHise', 17 => 'YmdHisO', 18 => 'YmdHisO', 19 => 'YmdHisO'];
    /**
     * @var string
     */
    protected $format = 'c';
    ...
    public function equals($value): bool
    {
        ...
        return $timestamp == $this->value->get_time_stamp();
    }
    ...
    public static function parse(string $content, ?Document $document = null, int &$offset = 0)
    {
        if (preg_match('/^\s*\(D\:(?P<name>.*?)\)/s', $content, $match)) {
            ...
            if (preg_match('/^\d{4}(\d{2}(\d{2}(\d{2}(\d{2}(\d{2}(Z(\d{2,4})?|[\+-]?\d{2}(\d{2})?)?)?)?)?)?)?$/', $name)) {
                ...
                $date = \DateTime::create_from_format($format, $name, new \DateTimeZone('UTC'));
            } else if (preg_match('/^\d{1,2}-\d{1,2}-\d{4},?\s+\d{2}:\d{2}:\d{2}[\+-]\d{4}$/', $name)) {
                $name = str_replace(',', '', $name);
                $format = 'n-j-Y H:i:sO';
                $date = \DateTime::create_from_format($format, $name, new \DateTimeZone('UTC'));
            }
            ...
            $offset += strpos($content, '(D:') + \strlen($match['name']) + 4;
            // 1 for '(D:' and ')'
