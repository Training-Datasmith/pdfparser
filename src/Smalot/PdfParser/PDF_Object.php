...
use Smalot\Pdf_Parser\Exception\Invalid_Dictionary_Object_Exception;
use Smalot\Pdf_Parser\X_Object\Form;
use Smalot\Pdf_Parser\X_Object\Image;
/**
 * Class PDFObject
 */
class Pdf_Object
{
     ...
     * @var bool
     */
    protected $add_position_whitespace = false;
    ...
    {
        return $this->header->get_details($deep);
    }
     ...
     * @internal
     */
    public function clean_content(string $content, string $char = 'X')
    {
        $char = $char[0];
    ...
    private function format_content(?string $content): string
    {
        ...
        $pdf_inline_images = [];
        $offset_bi = 0;
        while (preg_match('/\sBI\s(\/.+?)\sID\s(.+?)\sEI(?=\s|$)/s', $content, $text, \PREG_OFFSET_CAPTURE, $offset_bi)) {
            ...
            do {
                $para_test = $para;
                $para = preg_replace('/\(([^()]*)\)/', '$1', $para_test);
            } while ($para != $para_test);
            $para_open = strpos($para, '(');
            $para_close = strpos($para, ')');
            ...
            if (0 < $para_close && (false === $para_open || $para_close < $para_open)) {
            ...
            // dictionary
            $dict = $this->parse_dictionary('<<' . $text[1][0] . '>>');
            // Check if an image Width and Height are set in the dict
            if ((isset($dict['W']) || isset($dict['Width'])) && (isset($dict['H']) || isset($dict['Height']))) {
                $id = uniqid('IMAGE_', true);
                $pdf_inline_images[$id] = [preg_replace(['/\r\n/', '/\r/', '/\n/'], ' ', $text[1][0]), preg_replace(['/\r\n/', '/\r/', '/\n/'], '', $text[2][0])];
                $content = preg_replace('/' . preg_quote($text[0][0], '/') . '/', '^^^' . $id . '^^^', $content, 1);
            } else {
        ...
        while (preg_match('/' . preg_quote($attempt, '/') . '.*?\)/s', $content, $text)) {
            ...
            if (')' == $para[-1] && $left == $right) {
                ...
                $content = preg_replace('/' . preg_quote($text[0], '/') . '/', '@@@' . $id . '@@@', $content, 1);
        ...
        while (preg_match('/(<<.*?>> *)(BDC|BMC|DP|MP)/s', $content, $dicttext)) {
            ...
            $content = preg_replace('/' . preg_quote($dicttext[0], '/') . '/', ' ###' . $dictid . '###' . $dicttext[2], $content, 1);
        }
        ...
        $operators = ['b*', 'b', 'BDC', 'BMC', 'B*', 'BI', 'BT', 'BX', 'B', 'cm', 'cs', 'c', 'CS', 'd0', 'd1', 'd', 'Do', 'DP', 'EMC', 'EI', 'ET', 'EX', 'f*', 'f', 'F', 'gs', 'g', 'G', 'h', 'i', 'ID', 'I', 'j', 'J', 'k', 'K', 'l', 'm', 'MP', 'M', 'n', 'q', 'Q', 're', 'rg', 'ri', 'rI', 'RG', 'scn', 'sc', 'sh', 's', 'SCN', 'SC', 'S', 'T*', 'Tc', 'Td', 'TD', 'Tf', 'TJ', 'Tj', 'TL', 'Tm', 'Tr', 'Ts', 'Tw', 'Tz', 'v', 'w', 'W*', 'W', 'y', '\'', '"'];
        foreach ($operators as $operator) {
            $content = preg_replace('/(?<!\w|\/)' . preg_quote($operator, '/') . '(?![\w10\*])/', $operator . "\r\n", $content);
        }
        ...
        $pdf_inline_images = array_reverse($pdf_inline_images, true);
        foreach ($pdf_inline_images as $id => $image) {
     ...
     * @internal
     */
    public function get_sections_text(?string $content): array
    {
        $sections = [];
        // A cleaned stream has one command on every line, so split the
        // cleaned stream content on \r\n into an array
        $text_cleaned = preg_split('/(\r\n|\n|\r)/', $this->format_content($content), -1, \PREG_SPLIT_NO_EMPTY);
        $in_text_block = false;
        foreach ($text_cleaned as $line) {
            $line = trim($line);
            // Skip empty lines
            if ('' === $line) {
                continue;
    ...
    private function get_default_font(?Page $page = null): Font
    {
        $fonts = [];
        if (null !== $page) {
            $fonts = $page->get_fonts();
        }
        $first_font = $this->document->get_first_font();
        if (null !== $first_font) {
            $fonts[] = $first_font;
        }
        if (\count($fonts) > 0) {
     ...
     */
    private function get_tj_using_font_fallback(Font $font, array $command, ?Page $page = null, float $font_factor = 4): string
    {
        $orig_text = $font->decode_text($command, $font_factor);
        $text = $orig_text;
        // If we make this a Config option, we can add a check if it's
        // enabled here.
        if (null !== $page) {
            $font_ids = array_keys($page->get_fonts());
            ...
            while (preg_match('/[\x00-\x08\x0e-\x1f\x7f]/u', $text) || false !== strpos(bin2hex($text), '00')) {
                ...
                $font = $page->get_font(array_shift($font_ids));
                $text = $font->decode_text($command, $font_factor);
            }
        }
        return $text;
    }
     ...
     */
    public function parse_dictionary(string $dictionary): array
    {
        // Normalize whitespace
        $dictionary = preg_replace(['/\r/', '/\n/', '/\s{2,}/'], ' ', trim($dictionary));
        if ('<<' != substr($dictionary, 0, 2)) {
            throw new Invalid_Dictionary_Object_Exception('Not a valid dictionary object.');
        }
        $parsed = [];
        $stack = [];
        $current_name = '';
        $array_type_numeric = false;
        // Remove outer layer of dictionary, and split on tokens
        $split = preg_split('/(<<|>>|\[|\]|\/[^\s\/\[\]\(\)<>]*)/', trim(preg_replace('/^<<|>>$/', '', $dictionary)), -1, \PREG_SPLIT_NO_EMPTY | \PREG_SPLIT_DELIM_CAPTURE);
        foreach ($split as $token) {
            ...
            switch ($token) {
                ...
                case '[':
                    $parsed[$current_name] = [];
                    $array_type_numeric = true;
                    // Move up one level in the stack
                    $stack[\count($stack)] =& $parsed;
                    $parsed =& $parsed[$current_name];
                    $current_name = '';
                    break;
                // Open hashed array
                case '<<':
                ...
                case ']':
                    ...
                    if (\is_array($parsed) && 1 == \count($parsed) && isset($parsed[0]) && \is_string($parsed[0]) && '' !== $parsed[0] && '/' != $parsed[0][0]) {
                        $parsed = '[' . $parsed[0] . ']';
                    }
                // Close hashed array
                // no break
                ...
                default:
                    ...
                    if ('/' == substr($token, 0, 1)) {
                        $current_name = substr($token, 1);
                        if (true == $array_type_numeric) {
                            $parsed[] = $current_name;
                            $current_name = '';
                        }
                    } elseif ('' != $current_name) {
                        if (false == $array_type_numeric) {
                            $parsed[$current_name] = $token;
                        }
                        $current_name = '';
                    } elseif ('' == $current_name) {
                        $parsed[] = $token;
    ...
    {
        $this->add_position_whitespace = true;
        $result = $this->get_text_array($page);
        $this->add_position_whitespace = false;
     ...
     * @throws \Exception
     */
    public function get_text_array(?Page $page = null): array
    {
        $result = [];
        $text = [];
        $marked_stack = [];
        $last_written_position = false;
        $sections = $this->get_sections_text($this->content);
        $current_font = $this->get_default_font($page);
        $current_font_size = 1;
        $current_text_leading = 0;
        $current_position = ['x' => false, 'y' => false];
        $current_position_tm = ['a' => 1, 'b' => 0, 'c' => 0, 'i' => 0, 'j' => 1, 'k' => 0, 'x' => 0, 'y' => 0, 'z' => 1];
        $current_position_td = ['x' => 0, 'y' => 0];
        $current_position_cm = ['a' => 1, 'b' => 0, 'c' => 0, 'i' => 0, 'j' => 1, 'k' => 0, 'x' => 0, 'y' => 0, 'z' => 1];
        $clipped_font = [];
        $clipped_position_cm = [];
        self::$recursion_stack[] = $this->get_unique_id();
        foreach ($sections as $section) {
            $commands = $this->get_commands_text($section);
                ...
                switch ($command[self::OPERATOR]) {
                    ...
                    case 'BDC':
                        if (preg_match('/(<<.*>>)$/', $command[self::COMMAND], $match)) {
                            $dict = $this->parse_dictionary($match[1]);
                            ...
                            if (isset($dict['ActualText']) && \is_string($dict['ActualText']) && '' !== $dict['ActualText']) {
                                if ('[' == $dict['ActualText'][0]) {
                                    ...
                                    $marked_stack[] = ['ActualText' => $this->get_commands_text($dict['ActualText'] . 'TJ')[0]];
                    ...
                    case 'cm':
                        ...
                        $current_position_cm = ['a' => (float) $args[0], 'b' => (float) $args[1], 'c' => 0, 'i' => (float) $args[2], 'j' => (float) $args[3], 'k' => 0, 'x' => (float) $args[4], 'y' => (float) $args[5], 'z' => 1];
                        break;
                    case 'Do':
                        if (is_null($page)) {
                            break;
                        ...
                        if (\in_array($xobject->get_unique_id(), self::$recursion_stack, true)) {
                            break;
                        }
                        $object_text = $xobject->get_text($page);
                        // If the PDFObject is a Form and doesn't have any text,
                        // skip it.
                        if ($xobject instanceof Form && $object_text === ' ') {
                            break;
                        }
                        $text[] = $object_text;
                        break;
                            ...
                            switch ($action) {
                                ...
                                case 'ReversedChars':
                                    ...
                                    foreach ($text as $key => $t) {
                                        $text[$key] = implode('', array_reverse(mb_str_split($t, 1, mb_internal_encoding())));
                                    }
                    ...
                    case 'TJ':
                        ...
                        if (false === $actual_text && \is_array($last_written_position)) {
                            ...
                            $current_x = $last_written_position[0];
                            $current_y = $last_written_position[1];
                            $last_written_position = false;
                        } else {
                            $current_x = $current_position_cm['x'] + $current_position_tm['x'] + $current_position_td['x'];
                            $current_y = $current_position_cm['y'] + $current_position_tm['y'] + $current_position_td['y'];
                        }
                        $white_space = '';
                        $factor_x = -$current_font_size * $current_position_tm['a'] - $current_font_size * $current_position_tm['i'];
                        $factor_y = $current_font_size * $current_position_tm['b'] + $current_font_size * $current_position_tm['j'];
                        if (true === $this->add_position_whitespace && false !== $current_position['x']) {
                            $cur_y = $current_y - $current_position['y'];
                            if (abs($cur_y) >= abs($factor_y) / 4) {
                                $white_space = "\n";
                            } else {
                                if (true === $reverse_text) {
                                    $cur_x = $current_position['x'] - $current_x;
                                } else {
                                    $cur_x = $current_x - $current_position['x'];
                                }
                                ...
                                // option.
                                if ($cur_x >= abs($factor_x * 7)) {
                                    $white_space = "\t";
                                } elseif ($cur_x >= abs($factor_x * 2)) {
                                    $white_space = ' ';
                                }
                            }
                        }
                        $newtext = $this->get_tj_using_font_fallback($current_font, $command[self::COMMAND], $page, $factor_x);
                        ...
                        if (false === $actual_text) {
                            ...
                            if (false !== $reverse_text) {
                                ...
                                $text[] = preg_replace('/  $/', ' ', $newtext . $white_space);
                            } else {
                                // Otherwise add the whitespace first
                                if (' ' === $white_space && isset($text[\count($text) - 1])) {
                                    $text[\count($text) - 1] = preg_replace('/ $/', '', $text[\count($text) - 1]);
                                }
                                $text[] = preg_replace('/^[ \t]{2}/', ' ', $white_space . $newtext);
                            }
                            ...
                            $factor = $factor_x / 2;
                            $current_position = ['x' => $current_x - mb_strlen($newtext) * $factor, 'y' => $current_y];
                            ...
                            $last_written_position = [$current_x, $current_y];
                    ...
                    case 'TD':
                        ...
                        $current_position_td = ['x' => $current_position_td['x'] + $x * $current_position_tm['a'] + $x * $current_position_tm['i'], 'y' => $current_position_td['y'] + $y * $current_position_tm['b'] + $y * $current_position_tm['j']];
                        break;
                    case 'Tf':
                        $args = preg_split('/\s/s', $command[self::COMMAND]);
                    ...
                    case 'Tm':
                        $args = preg_split('/\s+/s', $command[self::COMMAND]);
                        $current_position_tm = ['a' => (float) $args[0], 'b' => (float) $args[1], 'c' => 0, 'i' => (float) $args[2], 'j' => (float) $args[3], 'k' => 0, 'x' => (float) $args[4], 'y' => (float) $args[5], 'z' => 1];
                        break;
                    default:
                }
            }
     ...
     */
    public function get_commands_text(string $text_part, int &$offset = 0): array
    {
        $commands = $matches = [];
        ...
        if ('TJ' == $operator) {
            ...
            do {
                ...
                if (preg_match('/^ *\((.*?)(?<![^\\\\]\\\\)\) *(-?[\d.]+)?/', $command, $tjmatch)) {
                    $subcommand[] = [self::TYPE => '(', self::OPERATOR => 'TJ', self::COMMAND => $tjmatch[1]];
                    if (isset($tjmatch[2]) && trim($tjmatch[2])) {
                        $subcommand[] = [self::TYPE => 'n', self::OPERATOR => '', self::COMMAND => $tjmatch[2]];
                    }
        ...
        $commands[] = [self::TYPE => $type, self::OPERATOR => $operator, self::COMMAND => $command];
        return $commands;
    }
    public static function factory(Document $document, Header $header, ?string $content, ?Config $config = null): self
    {
        switch ($header->get('Type')->get_content()) {
            case 'XObject':
                switch ($header->get('Subtype')->get_content()) {
                    case 'Image':
                        return new Image($document, $header, $config->get_retain_image_content() ? $content : null, $config);
                    case 'Form':
            ...
            case 'Font':
                $subtype = $header->get('Subtype')->get_content();
